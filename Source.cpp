...
public:
	...
	CerealaDietetica(int idNou, char* nume, string soi, int durataPerioadaVegetatie, bool consumUman, float temperatura, char* continentOrigine, float calorii, int fibre, bool areGluten) :Cereala(idNou, nume, soi, durataPerioadaVegetatie, consumUman, temperatura)
	{
	...
	CerealaDietetica(const Cereala& c) :Cereala(c)
	{
		if (continentOrigine)
		{
			delete[]this->continentOrigine;
		}
		this->continentOrigine = new char[strlen("Oceania") + 1];
		strcpy_s(this->continentOrigine, strlen("Oceania") + 1, "Oceania");
		this->calorii = 200;
		this->fibre = 15;
		this->areGluten = true;
	}
...
public:
	...
	void scriereInFisierBinar(fstream& f)
	{
		...
		for (int i = 0; i < this->nrLegumeGradina; i++)
		{
			...
			{
				f.write((char*)&this->legume[i].getNume()[j], sizeof(char));
			}
			...
			bool rs = this->legume[i].getRezistentaSeceta();
			f.write((char*)&rs, sizeof(bool));
			float pH = this->legume[i].getPHSol();
			f.write((char*)&pH, sizeof(float));
			float d = this->legume[i].getDistantaIntrePlante();
	...
	void citireDinFisierBinar(fstream& f)
	{
		...
		if (this->nrLegumeGradina > 0)
		{
			...
			for (int i = 0; i < this->nrLegumeGradina; i++)
			{
				...
				int lungime3;
				f.read((char*)&lungime3, sizeof(int));
				char* aux2 = new char[lungime3];
				for (int j = 0; j < lungime3; j++)
				{
